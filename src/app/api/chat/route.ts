...
//     contextualResponse +=
//       " JavaScript is a versatile programming language that powers modern web development, from frontend interactions to backend services.";
//   } else if (userMessage.toLowerCase().includes("react")) {
//     contextualResponse +=
//       " React is a popular JavaScript library for building user interfaces, especially for web applications with complex state management.";
//   } else if (userMessage.toLowerCase().includes("api")) {
//     contextualResponse +=
//       " APIs (Application Programming Interfaces) are essential for connecting different software systems and enabling data exchange between applications.";
//   } else {
//     contextualResponse +=
//       " This is definitely a topic worth exploring further, and there are many resources available to dive deeper into this subject.";
//   }
...
export async function POST(req: NextRequest) {
  try {
    ...
    let memories: Memory[] = [];
    try {
      const memoriesResult = await memoryManager.searchMemories(
        userId,
        userMessage.content
      );
    ...
    const memoryContext =
      memories.length > 0
        ? `Here's some relevant context from previous conversations: ${memories
            .map((m: Memory) => m.text)
            .join("\n")}`
    ...
    const contextMessages = contextManager.trimConversation([
      systemMessage,
      ...messages,
    ]);
